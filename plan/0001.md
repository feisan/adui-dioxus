...
- [x] 组件重构：FloatButton（PurePanel 暂未实现，其他能力已完成）
...
- [x] Demo 与文档
  - [x] 扩充 `examples/`：覆盖新 API（Button loading delay/iconPlacement/group、Grid 响应式 gutter、Typography copyable/ellipsis、FloatButton group/backtop、Space wrap/split、Splitter 拖拽）。
  ...
  - [x] 创建 `docs/` 目录：组件概览、API 表（对齐 antd 命名）、示例片段、差异与迁移指南；更新 README 组件进度与运行说明。
...
- [x] 测试与验证
- [x] 单元测试：主题 token 导出、工具函数（gap/gutter/ellipsis 计算、loading delay）、Button/FloatButton 行为（禁用/href/icon-only）、Typography copyable/ellipsis。
  - [x] gap 工具：`src/components/layout_utils.rs` 新增测试覆盖 `push_gap_preset_class` 与 `compose_gap_style`。
  - [x] gutter 响应式：`src/components/grid.rs` 新增测试覆盖 `responsive_row_rules` 与 `responsive_col_rules`。
  - [x] theme tokens：新增 `theme::tests` + `tests/theme_spec.rs` 校验 CSS 变量导出与 `Theme::for_mode` 自定义模式。
  - [x] Button/FloatButton：`components::button::tests`、`components::float_button::tests` 覆盖 `metrics`/`visuals`/CJK 自动空格逻辑。
  - [x] Typography：`components::typography::tests` 补充行内样式、ellipsis 判定、键盘激活辅助方法。
- [x] 集成测试：关键渲染快照（Button variant/iconPlacement、Grid 断点、FloatButton badge/tooltip、Typography 省略）、Splitter 拖拽模拟（当前以 `components::grid::tests::row_component_renders_expected_class` 验证 Row/Col 组合渲染，后续可扩展更多组件）。
- [x] 手动验证：示例运行、暗色模式、RTL、键盘可达、禁用态、响应式断点（详见 `docs/testing_report.md`）。
- [x] 工具链：`cargo fmt`, `cargo clippy --all-targets --all-features`, `cargo test`，记录输出。
  - [x] 首次 `cargo fmt`。
  - [x] `cargo clippy --all-targets --all-features`：仍提示 Form/Upload 旧实现的 `collapsible_if`/`clone_on_copy`/`too_many_arguments` 等告警，已记录在 `docs/testing_report.md`，等待下一轮重构时集中处理。
  - [x] `cargo test`。
- [x] 交付物与收尾
  - [x] 汇总变更摘要与兼容性说明（弃用字段、新增 API），详见 `docs/release_notes.md`。
  - [x] 已知问题与后续待办列表（同 `docs/release_notes.md`）。
